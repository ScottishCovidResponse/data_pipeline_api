...
public class ComponentsDeserializerTest {
  ...
  public void deserialize() throws JsonProcessingException {
    ...
    var distribution = ImmutableDistribution.builder().internalType(DistributionType.gamma).internalShape(1).internalScale(2).build();
    var samples = ImmutableSamples.builder().addSamples(1, 2, 3).build();
    var expectedComponents = ImmutableComponents.builder().components(Map.of("example-estimate", estimate, "example-distribution", distribution, "example-samples", samples)).build();
